...
def get_sign(functionId, body, client, clientVersion):
    ...
    # print(info)
    # print(ep)
    return 'functionId=%s&clientVersion=%s&client=%s&sdkVersion=31&lang=zh_CN&harmonyOs=0&networkType=wifi&oaid=%s&eid=%s&ef=1&ep=%s&st=%s&sign=%s&sv=%s' % (
        functionId, clientVersion, client, suid, randomeid(), urllib.parse.quote(ep), st, info, sv)


print(base64Decode('YWTwY2PvZNPwEQUmEJLuEWYzYtKmDQDwYWO4EJvrCNYzENCyENUzZq=='))
#
print(uuid.uuid4())